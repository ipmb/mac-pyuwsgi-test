...
RUN set -ex && apt-get update -q && apt-get install --no-install-recommends -y python3-pip python3-venv python3-dev build-essential
RUN python3.6 -m venv /pyuwsgi
WORKDIR /pyuwsgi
ARG NO_BINARY=:none:
RUN ./bin/pip install --no-binary=${NO_BINARY} pyuwsgi

# This is a default environment variable when running bash on Ubuntu 18.04. It's presence seems to be necessary for the segfault
...
CMD ./bin/pyuwsgi --socket=:0 --process=2 --procname-prefix-spaced=foo
